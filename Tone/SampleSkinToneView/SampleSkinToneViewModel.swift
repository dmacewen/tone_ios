...
class SampleSkinToneViewModel {
    ...
    let didFlashViewModelAppear = BehaviorSubject<Bool>(value: false)
        ...
        didFlashViewModelAppear
            .filter { $0 }
                //Observable.just(self.screenFlashSettings.count)
                    ...
                    .take(self.screenFlashSettings.count)
                    .toArray()
                    .map { faceCaptures -> [ImageData] in
                        
                        //let faceCaptures = faceCapturesOpt.map { $0!}
                    ...
                    .do(onCompleted: { DispatchQueue.main.async { self.events.onNext(.beginUpload) } } )
